...
## 2026-04-22 - [Requests Import Overhead in MultimediaManager]
**Learning:** Module-level `import requests` can introduce ~200-300ms of cumulative import overhead due to heavy dependencies like `charset_normalizer`, `urllib3`, and `cryptography`. This is particularly impactful for applications like the Gradio UI where multiple modules might pull in these dependencies.
**Action:** Defer `import requests` to a thread-safe lazy property (`session`). This reduced `multimedia.py` import time from ~295ms to ~49ms (~83% improvement) in the current environment.
