...
## 2026-05-20 - [Lazy Setup for CLI Responsiveness]
**Learning:** For CLI tools like `JudgeGuard`, module-level setup calls like `load_dotenv()` and `logging.basicConfig()` contribute ~30-40ms to pure import time, which is significant for high-frequency checks. Synchronous disk I/O in `__init__` (globbing, file reads) adds further latency that can be avoided if the specific paths or rules aren't needed for every execution path (e.g., cached hits).
**Action:** Implement a lazy `_ensure_setup()` method for one-time environment/logging configuration and refactor path discovery into thread-safe lazy properties. This ensures that the "pure import" cost is minimized and I/O is deferred until absolutely necessary.
