...
## 2026-04-18 - [Optimization of ResearchPipeline I/O and Regex]
**Learning:** For bulk file processing (e.g., 1000+ markdown files), the overhead of UTF-8 decoding during the hashing phase (to check for changes) is significant. Reading raw bytes for MD5 hashing and deferring decoding until a change is detected saves ~15-20% of 'no-change' scan time. Additionally, replacing nested `re.findall` and `re.search` with a single `re.finditer` pass and pre-compiled regex constants improves extraction throughput by ~20%.
**Action:** Use `read_bytes()` for content hashing in change-detection loops. Favor `re.finditer` for single-pass extraction from large text blocks to avoid intermediate list creation and redundant scanning.
