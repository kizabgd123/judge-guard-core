...
## 2026-04-22 - [Mock Patching with Dynamic Lazy Loading]
**Learning:** In Python benchmark tests that mock dependencies on dynamically lazy-loaded classes, standard module-level patching (e.g., patching `NotionClient` on a module that imports it inside method/property scopes) will fail with an `AttributeError`. They must be mock-patched at their original source modules, and property setters must be circumvented by writing directly to private members (e.g., `_notion`).
**Action:** Always patch lazily-imported dependencies at their original source modules (e.g., `src.antigravity_core.notion_client.NotionClient`). Set underlying private fields directly when property setters are unavailable.
