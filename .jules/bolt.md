...
## 2026-04-22 - [Lazy Imports and SQLite WAL in ResearchPipeline]
**Learning:** Top-level imports and synchronous SQLite writes in core utilities like `ResearchPipeline` create a performance floor that impacts all downstream CLI tools. Enabling WAL mode and deferring heavy imports (`requests`, `sqlite3`, `re`) can reduce module overhead by >70% and write latency by >90%.
**Action:** Use a lazy initialization pattern (e.g., `_ensure_setup`) for environment variables and logging. Defer heavy module imports to method scopes. Always enable `PRAGMA journal_mode=WAL` for SQLite databases in multi-threaded or high-frequency write environments.
