...
## 2026-04-24 - [Mock Patching with Lazy Module Imports]
**Learning:** Adding lazy loading to heavy classes (like `NotionClient` and `GeminiClient` in properties) creates a performance improvement, but breaks unit tests and benchmarks that attempt to mock-patch those classes at the importer's namespace (e.g., `@patch('src.kaggle_stream.kaggle_agent.NotionClient')`). Because the class is imported inside the local property method rather than at the module level, the module does not possess that attribute.
**Action:** Patch those objects directly at their original source paths (e.g., `@patch('src.antigravity_core.notion_client.NotionClient')`) and mock any instances directly on private properties (e.g., `agent._notion = mock`) if property setters are not available.
