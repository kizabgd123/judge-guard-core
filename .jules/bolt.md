...
## 2026-04-25 - [Lazy Loading Modifies Mock Patching Targets]
**Learning:** When modules are optimized to lazy-load dependencies internally (to reduce initial startup overhead), they no longer contain those dependencies as module-level attributes on import. As a result, standard unit and benchmark tests that attempt to patch the class target at the importing module level (e.g., `@patch('src.kaggle_stream.kaggle_agent.NotionClient')`) will raise `AttributeError`.
**Action:** Always patch the dependency at its original source path (e.g., `@patch('src.antigravity_core.notion_client.NotionClient')`) when the target importing module utilizes lazy loading patterns.
