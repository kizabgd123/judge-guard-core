...
## 2026-05-20 - [CLI Startup Latency and Lazy Setup]
**Learning:** For high-frequency CLI tools, even "standard" library overhead (logging, glob, concurrent.futures) and configuration steps like `load_dotenv()` can account for >50% of the execution time on hot paths (e.g., cache hits). Deferring these to a lazy `_ensure_setup()` method significantly improves user-perceived responsiveness.
**Action:** Always move module-level configuration (logging, dotenv) and heavy imports into lazy properties or local scopes for CLI-facing components.
