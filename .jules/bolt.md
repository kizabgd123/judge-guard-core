...
## 2026-04-22 - [Redundant Tail Reads in CLI Verification]
**Learning:** Checking recent log entries and fetching verification context from `WORK_LOG.md` independently leads to redundant disk seeks, reads, and UTF-8 string decoding of the same content. On large files (e.g. 10MB+), this can add noticeable overhead during the high-frequency CLI path.
**Action:** Implement `_get_work_log_tail` inside `JudgeGuard` with a caching layer validated by file path, size, and modification time (`mtime`). Ensure the cache is only hit when the cached content length is at least the requested `max_chars` or covers the entire file size to prevent truncation of larger subsequent read requests.
