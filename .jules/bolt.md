...
## 2026-05-22 - [Import Overhead of Network Libraries]
**Learning:** Importing `requests` at the module level can add ~300-400ms to a module's import time, which is disproportionately large for CLI tools or background workers that might only need network access conditionally.
**Action:** Defer `requests` initialization using a thread-safe lazy property (with `threading.Lock`) and local imports. This reduced `MultimediaManager` import overhead by ~89% (311ms -> 32ms).
