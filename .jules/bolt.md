...
## 2026-04-24 - [Mocking Session Objects in Ad-Hoc Caching Tests]
**Learning:** Refactoring networking classes to use `requests.Session` for performance can break existing mock tests that patch module-level `requests.post`. Such tests must be updated to patch `requests.Session.post` or `requests.Session.request` to align with the new connection-pooling execution path.
**Action:** When updating networking code to use `Session`, scan for and update any mock paths in tests and verification scripts from `requests.post` to `requests.Session.post`.
