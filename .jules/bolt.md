...
## 2026-04-24 - [Negative Verdict Lookup Caching and Failure State Tracking]
**Learning:** In `ResearchPipeline`, querying SQLite for un-cached actions (`get_cached_verdict`) returned `None` without caching the miss in memory, causing repeated queries for un-cached actions to continually hit disk (~0.02ms per query). In `JudgeGuard` and `KaggleAgent`, lazy properties missing failure flags re-attempted lock acquisition, imports, and exception handling on every access when optional dependencies were unavailable (~34ms / 1000 calls).
**Action:** Store `None` in `_verdict_cache` on database misses to enable O(1) in-memory returns (~0.0001ms, ~200x speedup), and add failure-tracking flags (`_gemini_failed`, `_pipeline_failed`, `_notion_failed`) on lazy properties to short-circuit repeated failed initializations (~0.08ms / 1000 calls, ~400x speedup).
