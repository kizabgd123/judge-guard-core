...
## 2026-05-15 - [Gradio App Startup Latency and Mock Compatibility]
**Learning:** Importing `gradio` and initializing AI agents eagerly in `src/kaggle_stream/app.py` creates a massive ~4.5s startup delay. Implementing lazy loading via module-level `__getattr__` reduces this to <30ms, but requires careful handling of `threading.RLock` to avoid deadlocks in nested resource calls (like `demo` needing `gr`). Furthermore, lazy-loading must explicitly check `globals()` to remain compatible with `unittest.mock.patch` which operates by replacing module-level attributes.
**Action:** Use `threading.RLock` for all lazy resource registries to support nested initialization. Ensure the resource resolver checks `globals()` before the internal cache to support unit testing and mocking. Defer interface construction to a `create_demo` helper.
