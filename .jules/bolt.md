...
## 2026-04-22 - [Heavy Import Overhead in MultimediaManager]
**Learning:** Global imports of `requests` (~200ms) and `logging` (~24ms) in `multimedia.py` directly impact the startup time of any application using it, such as the Kaggle Stream Gradio app. This overhead is particularly noticeable when the module is imported but not immediately used for network calls.
**Action:** Implement thread-safe lazy properties (`session`, `logger`) using `threading.RLock` and move heavy imports into property getters. This reduced `multimedia.py` import time by ~94% (305ms -> 19ms) and deferred the ~200ms `requests` overhead until the first API call.
