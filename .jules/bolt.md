...
## 2026-04-24 - [SQLite Fsync Latency and Cold Cache Lookups]
**Learning:** Default SQLite configuration uses `journal_mode=DELETE` and `synchronous=FULL`, forcing disk fsync operations on every commit (e.g., ~3.32ms per `cache_verdict` write). Additionally, instantiating `ResearchPipeline` without pre-populating `_verdict_cache` forces initial SQLite disk queries (~292 µs) on verdict lookups.
**Action:** Configure `PRAGMA journal_mode=WAL;` and `PRAGMA synchronous=NORMAL;` on SQLite connection setup to reduce write latency by ~95% (~0.15ms per write). Pre-populate `_verdict_cache` during connection initialization via `_load_verdict_cache()` to reduce first-read latency by ~95% (~12.4 µs).
