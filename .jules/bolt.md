...
## 2026-04-25 - [Stat-Based Caching for Streamed Log Tails]
**Learning:** High-frequency polling methods like `LogStreamer.get_context()` perform file opens, seek-from-end operations, reads, and UTF-8 decodes on every call, even when the underlying log file (`WORK_LOG.md`) has not changed. Checking filesystem metadata (`st_dev`, `st_ino`, `st_mtime_ns`, `st_size`) via `os.stat()` before reading bypasses disk I/O and decoding entirely on cache hits.
**Action:** Cache the log context along with `st_dev`, `st_ino`, `st_mtime_ns`, and `st_size`. On subsequent calls, validate `os.stat()` against cached values before opening the file.
