...
## 2026-04-25 - [Notion Title Extraction and Priority Matching Overhead]
**Learning:** Notion's API schema properties can be deeply nested, and performing linear searches with generator expressions on page objects (like scanning all properties for a specific title identifier) can introduce significant latency and memory allocation overhead. Additionally, falling back to rich text property types can bypass correct identifier checking.
**Action:** Implement fast-path direct dictionary lookups for expected title property names, and only fall back to a full linear scan if the common property keys are absent, ensuring that priority hierarchies (like checking `id == "title"` before rich text fallback) are strictly preserved.
