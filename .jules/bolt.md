...
## 2026-04-18 - [Thread-Safe Lazy Loading and Background Warmup]
**Learning:** Concurrent property access during background warmup (e.g., triggering a heavy import in a background thread) can lead to race conditions if the lazy initialization logic is not thread-safe. This can result in multiple initializations or broken state.
**Action:** Use a `threading.Lock` to protect lazy initialization in multi-threaded components. Overlap heavy resource warmup with other I/O-bound tasks to hide latency; in `GuardianAgent`, this reduced cold-start latency by ~70%.
