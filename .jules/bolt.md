...
## 2026-04-22 - [SQLite WAL Mode and Lazy Loading for ResearchPipeline]
**Learning:** Module-level imports of heavy libraries like `sqlite3`, `requests`, and `dotenv` can add ~25ms+ to import time. Synchronous SQLite writes without WAL mode can take ~2ms per entry, which adds up in bulk operations.
**Action:** Implement lazy loading for all heavy dependencies and use SQLite `PRAGMA journal_mode=WAL` and `PRAGMA synchronous=NORMAL` to reduce write latency. This achieved a ~96% reduction in audit log latency (~1.97ms -> ~0.08ms) and a ~39% reduction in module import time (~67ms -> ~41ms).
