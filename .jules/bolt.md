...
## 2026-07-14 - [CI Safety for Browser Workers]
**Learning:** Accessing 'document' or 'window' properties (like 'visibilityState' or 'getElementById') in non-browser environments (e.g., Cloudflare Workers during CI builds) can cause fatal crashes or build failures. These environments may use proxies that throw on property access rather than returning undefined.
**Action:** Always guard DOM accesses with 'typeof document !== "undefined"' and wrap property accesses in 'try...catch' blocks for robustness in restricted CI environments.
