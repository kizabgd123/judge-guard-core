...
## 2026-04-22 - [Startup Latency from Heavy Third-Party Imports]
**Learning:** Importing heavy libraries like `gradio` (~3.2s) and `requests` (~180ms) at the module level significantly delays application startup and tool responsiveness. For CLI-accessible modules or web apps with multiple entry points, these costs accumulate and degrade the user experience.
**Action:** Implement module-level `__getattr__` and thread-safe lazy resource registries to defer heavy imports and global object instantiation until they are actually needed. Offloading `requests.Session` to a lazy property in utility classes further optimizes the hot path for other modules.
