...
## 2026-04-25 - [Overhead of Dict Unpacking in Fallback Scans]
**Learning:** Adding $O(1)$ fast-path checks for expected keys in highly nested dictionaries (like Notion objects) is an effective performance boost, but if a fallback linear scan is still needed, reusing the original `.items()` unpacking (`(k, v)`) loop can introduce unexpected performance degradation. Bypassing key unpacking by iterating over `.values()` directly and combining it with upfront direct dictionary accessors significantly speeds up both common and edge-case execution paths.
**Action:** Always prefer `.values()` over `.items()` when the key is unused during dictionary value sweeps, and structure fast-path checks to exit early before fallback loops run.
