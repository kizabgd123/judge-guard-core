...
## 2026-05-15 - [Import Overhead of Network Libraries]
**Learning:** Heavy libraries like `requests` can add ~200ms to module import time, which accumulates and slows down application startup, especially in multi-agent environments.
**Action:** Use thread-safe lazy properties (`threading.RLock`) to defer importing heavy network libraries until they are actually needed. This reduced `MultimediaManager` import time by ~99% (434ms -> 3ms). Always implement `close()` and `__del__` for proper session cleanup.
