...
## 2026-04-22 - [Module-level Lazy Loading in Gradio Apps]
**Learning:** Eagerly importing `gradio` and initializing thread pools/agents at the module level can add ~3-5s of latency to script startup. This is unacceptable for multi-entrypoint modules where only some paths require the UI.
**Action:** Use module-level `__getattr__` and a thread-safe resource registry to defer heavy imports and global object instantiation. Encapsulate UI definitions in functions. This reduced `src.kaggle_stream.app` import time from ~4.6s to ~0.03s (~99% reduction).
