...
## 2026-04-25 - [Stat-Based Caching in LogStreamer Context Retrieval]
**Learning:** Uncached context fetching in stream loops (`LogStreamer.get_context()`) incurs repeated file open, seek, read, and UTF-8 decode operations (~30.85 µs per call). Validating an in-memory context cache against `os.stat` attributes (`st_dev`, `st_ino`, `st_mtime_ns`, `st_size`) eliminates disk reads on unchanged log files (~3.84 µs per call, ~87.5% latency reduction).
**Action:** Implement `_cache` storing `(stat_key, content)` in file reading utilities. Check `os.stat` before opening files to serve hits from cache instantly while maintaining instant invalidation when files are modified or appended.
