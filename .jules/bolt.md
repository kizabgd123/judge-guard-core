...
## 2026-04-22 - [Optimized ResearchPipeline Instantiation and SQLite Writes]
**Learning:** Initializing `ResearchPipeline` and performing synchronous SQLite writes in the `cache_verdict` hot-path can be significantly optimized. Instantiation overhead can be reduced by ~97% (0.18ms -> 0.01ms) by deferring `ThreadPoolExecutor` and `load_dotenv` imports. SQLite write latency can be reduced by ~97% (3.7ms -> 0.13ms) by enabling WAL mode and setting `synchronous=NORMAL`.
**Action:** Always enable WAL mode for SQLite databases used by agents. Defer heavy imports like `concurrent.futures` and `dotenv` to lazy properties or method scopes, and use `threading.RLock` for thread-safe lazy resource initialization.
