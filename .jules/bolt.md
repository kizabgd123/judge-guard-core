...
## 2026-04-22 - [Module-level __getattr__ inside Local Namespace]
**Learning:** In Python, accessing a lazily-loaded module-level attribute as a bare name from within the same module's functions does not trigger `__getattr__` because local lookup of globals bypasses module attribute lookups and directly throws a `NameError`.
**Action:** Always access deferred attributes via a central retriever/resource helper or `getattr(sys.modules[__name__], name)` to ensure module-level lookup is triggered correctly.
