...
## 2026-04-25 - [Repeated Failure Initialization Overhead in Lazy Properties]
**Learning:** When lazy property initialization fails due to missing optional dependencies or credentials (e.g., `ResearchPipeline` or `NotionClient`), returning `None` without caching the failure causes every subsequent property access to re-attempt the import, module resolution, and exception handling. When accessed repeatedly in tight loops or polling methods, this uncached failure handling incurs ~22ms of cumulative latency per 1000 accesses.
**Action:** Use failure-tracking boolean flags (`_notion_failed`, `_gemini_failed`, `_pipeline_failed`) inside lazy property getters to short-circuit repeated failed instantiation attempts (~100x latency reduction from ~22µs to ~0.17µs per lookup).
