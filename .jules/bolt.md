...
## 2026-04-24 - [Linear Scan and Generator Overhead in Title Resolution]
**Learning:** Using generic linear scans with generator expressions inside `next()` to extract a property value (e.g., matching `"id" == "title"` in Notion page properties) adds considerable latency and memory allocation overhead. Additionally, when page dictionaries don't conform to the expected format (common in mocks or tests), accessing missing keys directly raises exceptions, which degrades performance by falling back to slow Python `except Exception` handling.
**Action:** Implement O(1) dictionary checks for common key fast-paths (`"Name"`, `"Title"`, `"Entry"`) using `.get()` before running full linear scans. Always check property types safely via `isinstance` instead of relying on slow `try/except` exceptions for parsing keys.
