...
## 2026-04-24 - [SQLite WAL Mode, Lazy ThreadPoolExecutor, and Negative Caching in ResearchPipeline]
**Learning:** Default SQLite configuration uses `journal_mode = DELETE` and `synchronous = FULL`, causing disk fsync waits (~1.9ms per commit). Enabling `PRAGMA journal_mode=WAL;` and `PRAGMA synchronous=NORMAL;` reduces write commit latency to ~0.06ms (~30x faster). Furthermore, instantiating `ThreadPoolExecutor` and calling `load_dotenv()` on every class `__init__` adds ~0.08ms per instance. Lazy-loading `executor` and thread-safe process-level `load_dotenv()` reduces class instantiation latency by ~170x (from 0.085ms to 0.0005ms). Finally, storing negative query results (`None`) in `_verdict_cache` avoids repeated SQLite queries for un-cached actions, speeding up repeat misses by ~10x.
**Action:** Always enable SQLite WAL mode and `synchronous=NORMAL` for write-heavy SQLite clients. Use thread-safe lazy property initialization for `ThreadPoolExecutor` and cache negative query results in in-memory lookup structures.
