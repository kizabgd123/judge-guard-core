...
## 2026-04-24 - [Negative Caching for Verdict Cache Misses]
**Learning:** Querying SQLite on every verdict lookup cache miss in `ResearchPipeline.get_cached_verdict` adds database query latency (~2.78 µs) repeatedly when checking un-cached actions. Storing negative query results (`_verdict_cache[action_hash] = None`) allows repeat lookups for un-cached actions to hit the O(1) in-memory cache (~0.22 µs), achieving a ~12.4x speedup (~92% reduction in latency) without compromising cache updates.
**Action:** Store `None` in the in-memory cache dictionary when database queries return `None` on cache misses, and ensure `cache_verdict` overwrites negative entries when a new verdict is recorded.
