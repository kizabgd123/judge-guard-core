...
## 2026-04-24 - [O(1) Checks in Notion Title Extraction and Lazy Import Mock Patching]
**Learning:** Checking dynamic Notion properties by iterating and evaluating key/value pairs using `next()` can be slow and brittle, especially when mock/benchmark datasets lack expected keys (like "id"), which causes Python to raise and catch exceptions. This exception-handling overhead significantly slows down the path (~61% overhead). Additionally, lazy loading of classes (like `NotionClient`) prevents standard destination-path mocking, requiring tests to patch original source definitions.
**Action:** Always perform direct O(1) dictionary key retrievals safely with `.get()` first to prevent exception raising. When classes are lazily-imported within properties, mock-patch them at their original definition paths (e.g., `src.antigravity_core.notion_client.NotionClient`) and assign mocks to the underlying private properties (e.g., `_notion`).
