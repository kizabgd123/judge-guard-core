...
## 2026-04-22 - [SQLite Write Latency and Concurrency]
**Learning:** Default SQLite configuration (`DELETE` journal mode and `FULL` synchronous) results in high write latency (approx 4-6ms per single-row insert) because every commit forces a synchronous disk flush. This bottleneck scales linearly with the number of transactions.
**Action:** Enable `PRAGMA journal_mode=WAL` and `PRAGMA synchronous=NORMAL` to reduce disk wait times and improve concurrency. Wrap every database interaction in `threading.RLock` to ensure thread-safety when `check_same_thread=False` is used. This reduced `ResearchPipeline.cache_verdict` latency by ~97% (4.6ms -> 0.14ms).
