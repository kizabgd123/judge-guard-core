...
## 2026-04-24 - [SQLite WAL Mode and Lazy Resource Allocation in ResearchPipeline]
**Learning:** Default SQLite connections use `DELETE` journal mode with `synchronous=FULL`, causing every `commit()` during audit/verdict writes to block on synchronous disk flushes (~4.1ms per write). Enabling `PRAGMA journal_mode=WAL;` and `PRAGMA synchronous=NORMAL;` reduces write/commit latency by ~25x (~0.16ms per write). Deferring `ThreadPoolExecutor` creation and caching `load_dotenv` loading via thread-safe class locking further reduces class instantiation overhead by >75x.
**Action:** Always enable SQLite WAL mode and `synchronous=NORMAL` on embedded database initialization/connection, and defer ThreadPoolExecutor creation to lazy properties.
