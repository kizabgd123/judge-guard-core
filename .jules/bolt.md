...
## 2026-04-22 - [SQLite Write Latency and Import Overhead in ResearchPipeline]
**Learning:** Default SQLite settings (journal_mode=DELETE, synchronous=FULL) can make simple writes take ~5ms, which is significant when auditing high-frequency actions. Additionally, global imports of `concurrent.futures` and `dotenv` contribute to module-level overhead.
**Action:** Enable `journal_mode=WAL` and `synchronous=NORMAL` in SQLite to reduce write latency (observed ~97% reduction, 4.6ms -> 0.12ms). Defer `ThreadPoolExecutor` and `load_dotenv` to lazy properties/methods to shave ~12ms off import time.
