...
## 2026-04-22 - [Module-Level __getattr__ Local Scope Bypass]
**Learning:** In Python, accessing a lazily-loaded module-level attribute from within the same module's functions does not trigger `__getattr__` because local lookup of globals bypasses module attribute lookups.
**Action:** To correctly access deferred attributes within inner functions of the same module and ensure lazy loading/mock-compatibility, retrieve them directly from the module's registration helper (e.g., calling `_get_resource("name")`) or fetch them via a module object lookup.
