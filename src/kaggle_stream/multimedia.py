...
class MultimediaManager:
    ...
    def __init__(self, hf_token: Optional[str] = None):
        ...
        # ⚡ Bolt: On-disk write avoidance tracking to prevent redundant I/O
        self._audio_file_cache = {}  # {output_path: text}
        self._image_file_cache = {}  # {output_path: mood}

    @property
    def session(self):
        """⚡ Bolt: Thread-safe lazy loading of requests.Session to minimize module import overhead."""
    ...
    def generate_audio(self, text: str, output_path: str = "speech.mp3"):
        # ⚡ Bolt: On-disk write avoidance - if file exists and tracks correct text, skip completely
        if text in self._audio_cache and os.path.exists(output_path) and self._audio_file_cache.get(output_path) == text:
            logger.info(f"⚡ Bolt: Correct audio file already exists on disk at {output_path}, skipping write.")
            return output_path

        # ⚡ Bolt: Cache check - if text was already generated but not written/different path, write cached bytes
    ...
    def generate_mood_image(self, mood: str, output_path: str = "mood.png"):
        # ⚡ Bolt: On-disk write avoidance - if file exists and tracks correct mood, skip completely
        if mood in self._image_cache and os.path.exists(output_path) and self._image_file_cache.get(output_path) == mood:
            logger.info(f"⚡ Bolt: Correct mood image already exists on disk at {output_path}, skipping write.")
            return output_path

        # ⚡ Bolt: Cache check - if mood icon was already generated but not written/different path, write cached bytes
