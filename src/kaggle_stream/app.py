...
def run_agent_turn(agent, task, context=""):
    ...
    # audio and image generation are independent network-bound tasks.
    with ThreadPoolExecutor(max_workers=2) as executor:
        audio_future = executor.submit(
            multimedia.generate_audio, message, f"{agent.name}_speech.mp3"
        )
        image_future = executor.submit(
            multimedia.generate_mood_image, f"{mood} mascot", f"{agent.name}_mood.png"
        )
