...
# ⚡ Bolt: Centralized resource registry for lazy loading
_lazy_resources = {}
_resource_lock = threading.Lock()

def _get_resource(name):
    """
    Internal helper to resolve and cache lazy resources.
    ⚡ Bolt: Prevents expensive imports and object creation until absolutely necessary.
    """
    # Fast path: check if already initialized
    if name in _lazy_resources:
        return _lazy_resources[name]

    # Check globals as well (in case of manual injection/tests)
    if name in globals():
        return globals()[name]
...
def __getattr__(name):
    """
    ⚡ Bolt: Python 3.7+ module-level __getattr__ for lazy resource loading.
    Allows external access to agent_alpha, agent_beta, multimedia, and executor
...
def run_agent_turn(agent, task, context="", return_futures=False):
    ...
    # ⚡ Bolt: Access resources via internal getter to ensure lazy initialization
    exec_inst = _get_resource("executor")
    mm_inst = _get_resource("multimedia")

    # ⚡ Bolt: Parallelize multimedia generation to reduce latency
    audio_future = exec_inst.submit(mm_inst.generate_audio, message, f"{agent.name}_speech.mp3")
    image_future = exec_inst.submit(mm_inst.generate_mood_image, f"{mood} mascot", f"{agent.name}_mood.png")
...
def collaborative_step(mode, task):
    ...
    if mode == "Project Log Stream":
        # ⚡ Bolt: Defer LogStreamer import until it's actually needed
    ...
    # ⚡ Bolt: Access agents via internal getter
...
def launch_app():
    """
    ⚡ Bolt: Encapsulate Gradio UI construction to avoid 'import gradio' at module level.
    Reduces module import time significantly.
    """
    import gradio as gr
