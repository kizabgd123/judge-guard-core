...
def collaborative_step(mode, task):
    ...
    ⚡ Bolt: Turn-level pipeline parallelization for both agents.
    Alpha and Beta multimedia generation tasks now run concurrently across thread pool workers.
    """
    current_task = task
    ...
    # 2. Start Beta (Reasoning happens while Alpha's Audio/Images are generating; returns futures for Beta's multimedia)
    msg_b, img_fut_b, aud_fut_b, thought_b = run_agent_turn(agent_beta, current_task, context=thought_a, return_futures=True)

    # 3. Finalize assets for both agents concurrently across thread pool workers
