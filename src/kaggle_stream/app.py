...
# ⚡ Bolt: Global lazy resource registry and lock to ensure thread safety
_lazy_resources = {}
_lock = threading.Lock()

def _get_resource(name):
    """⚡ Bolt: Retrieve or lazy-initialize a resource thread-safely."""
    # Check globals() first to respect any mock patching in test environments
    if name in globals():
        return globals()[name]
...
def __getattr__(name):
    """⚡ Bolt: Enable module-level lazy attribute resolution."""
...
def run_agent_turn(agent, task, context="", return_futures=False):
    ...
    # ⚡ Bolt: Use lazy loaded resources instead of module-level globals
...
def _build_demo():
    """⚡ Bolt: Defer build of Gradio Interface to reduce startup overhead."""
    import gradio as gr
