...
# ⚡ Bolt: Registry for lazily-loaded resources
_lazy_resources = {}
...
def _get_resource(name):
    ...
    with _resource_lock:
        ...
        # Check if it was manually patched in globals() (for tests)
...
def __getattr__(name):
    """⚡ Bolt: Support external access to lazily-loaded resources (PEP 562)."""
...
def run_agent_turn(agent, task, context="", return_futures=False):
    ...
    # ⚡ Bolt: Use lazy resource getters for internal references
    exec_inst = _get_resource("executor")
    multi_inst = _get_resource("multimedia")

    # ⚡ Bolt: Parallelize multimedia generation to reduce latency
    audio_future = exec_inst.submit(multi_inst.generate_audio, message, f"{agent.name}_speech.mp3")
    image_future = exec_inst.submit(multi_inst.generate_mood_image, f"{mood} mascot", f"{agent.name}_mood.png")
...
def _build_demo():
    """⚡ Bolt: Defer gradio import and UI construction until demo access."""
    import gradio as gr
...
if __name__ == "__main__":
    # ⚡ Bolt: Accessing demo triggers lazy initialization and gradio import
