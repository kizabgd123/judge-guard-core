...
# ⚡ Bolt: Lazy resource registry with thread safety
_lazy_resources = {}
_resource_lock = threading.Lock()

def _get_resource(name: str) -> Any:
    """Helper to initialize and cache resources on demand."""
    # Fast path: check if already in globals (e.g. if patched in tests)
    # or if already in our lazy cache.
...
def __getattr__(name: str) -> Any:
    """⚡ Bolt: Handle module-level lazy loading for external access (e.g. tests)."""
...
def run_agent_turn(agent, task, context="", return_futures=False):
    ...
    # Use _get_resource to ensure lazy initialization
...
def collaborative_step(mode, task):
    ...
    # ⚡ Bolt: Get agents via lazy loader
    ...
    # ⚡ Bolt: Return futures for Beta as well to maximize pipeline parallelization
    msg_b, img_fut_b, aud_fut_b, thought_b = run_agent_turn(beta, current_task, context=thought_a, return_futures=True)

    # 3. Finalize all assets in parallel
...
def launch_app():
    """⚡ Bolt: Encapsulate Gradio interface to defer heavy import."""
    import gradio as gr
