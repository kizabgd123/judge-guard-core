...
# ⚡ Bolt: Lazy resource registry for thread-safe on-demand initialization
_lazy_resources = {}
_resource_lock = threading.Lock()

def _get_resource(name):
    """Internal helper to access lazy resources safely."""
    ...
    with _resource_lock:
        ...
        # Check globals() first (for tests that might have patched them)
...
def __getattr__(name):
    """⚡ Bolt: Python 3.7+ module-level getattr for lazy loading."""
...
def run_agent_turn(agent, task, context="", return_futures=False):
    ...
    # ⚡ Bolt: Access lazy resources through helper
    exec_tool = _get_resource("executor")
    mm_tool = _get_resource("multimedia")

    # ⚡ Bolt: Parallelize multimedia generation to reduce latency
    audio_future = exec_tool.submit(mm_tool.generate_audio, message, f"{agent.name}_speech.mp3")
    image_future = exec_tool.submit(mm_tool.generate_mood_image, f"{mood} mascot", f"{agent.name}_mood.png")
...
def _build_demo():
    """⚡ Bolt: Helper to build Gradio interface on demand."""
    import gradio as gr
...
if __name__ == "__main__":
    # Accessing 'demo' attribute triggers lazy loading
    demo.launch()
