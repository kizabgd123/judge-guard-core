...
# ⚡ Bolt: Lazy resource registry and lock
_lazy_resources = {}
_init_lock = threading.Lock()

def _get_resource(name):
    """⚡ Bolt: Thread-safe registry for deferred resource initialization."""
    # Check if already initialized in globals or registry
    if name in globals():
        return globals()[name]
        ...
        with _init_lock:
            if name not in _lazy_resources:
                if name == "agent_alpha":
                    ...
                    import gradio as gr
                    _lazy_resources[name] = _build_demo(gr)
    return _lazy_resources.get(name)

def __getattr__(name):
    """⚡ Bolt: Support external access to lazily-loaded module-level resources."""
    resource = _get_resource(name)
...
def run_agent_turn(agent, task, context="", return_futures=False):
    ...
    # ⚡ Bolt: Use explicit getters for internal references
    exec_obj = _get_resource("executor")
    mm = _get_resource("multimedia")

    # ⚡ Bolt: Parallelize multimedia generation to reduce latency
    audio_future = exec_obj.submit(mm.generate_audio, message, f"{agent.name}_speech.mp3")
    image_future = exec_obj.submit(mm.generate_mood_image, f"{mood} mascot", f"{agent.name}_mood.png")
...
def collaborative_step(mode, task):
    ...
    # ⚡ Bolt: Use explicit getters for internal references to avoid NameError
...
def _build_demo(gr):
    """⚡ Bolt: Separate interface building logic to support lazy loading."""
