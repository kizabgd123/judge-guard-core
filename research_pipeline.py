...
class ResearchPipeline:
    ...
    def parse_markdown_files(self) -> List[int]:
        ...
        ⚡ Bolt: Pre-fetches hashes to avoid O(N) queries and returns affected IDs.
        """
        if not self.conn:
            self.connect()
        ...
        # ⚡ Bolt: Pre-fetch all filename/hash pairs to eliminate per-file SELECT overhead
        existing_hashes = {
        ...
        for md_path in md_files:
            ...
            if existing_hashes.get(filename_str) == content_hash:
            ...
            # Upsert document and return ID for targeted pattern extraction
            # ⚡ Bolt: SQLite 3.50+ supports RETURNING clause
    ...
    def extract_patterns(self, doc_ids: Optional[List[int]] = None):
        ...
        ⚡ Bolt: Supports targeted extraction via doc_ids and uses executemany for batch performance.
        """
        if not self.conn:
            self.connect()
        
        if doc_ids:
            # ⚡ Bolt: Targeted re-extraction for modified documents
            query = "SELECT id, content FROM documents WHERE id IN ({})".format(
                ",".join(["?"] * len(doc_ids))
            )
            docs = self.conn.execute(query, doc_ids).fetchall()

            # ⚡ Bolt: Delete old patterns for these documents before re-inserting
            delete_query = "DELETE FROM patterns WHERE doc_id IN ({})".format(
                ",".join(["?"] * len(doc_ids))
            )
            self.conn.execute(delete_query, doc_ids)
        else:
            # Full scan
        ...
        if patterns_to_insert:
            # ⚡ Bolt: Use executemany for O(1) batch insertion overhead
...
def main():
    ...
    if args.init:
        ...
        # ⚡ Bolt: Only extract patterns for documents that were actually changed
        patterns = 0
        if affected_ids:
