...
# === CONFIG (⚡ Bolt: Using strings and os.path to avoid pathlib import overhead) ===
DB_PATH = "./research.db"
...
class ResearchPipeline:
    ...
    @property
    def pattern_re(self):
        """⚡ Bolt: Lazy-load regex."""
        if ResearchPipeline._PATTERN_RE is None:
            import re
            ResearchPipeline._PATTERN_RE = re.compile(r"^###?\s+(?:\d+\.\s+)?(.+?)(?:\s*[-–]\s*(.+))?$", re.MULTILINE)
        return ResearchPipeline._PATTERN_RE
    ...
    def sync_to_notion(self):
        ...
        if not token or not db_id:
            ...
            notion_log_dir = os.path.dirname(str(NOTION_LOG))
            if not os.path.exists(notion_log_dir):
                os.makedirs(notion_log_dir, exist_ok=True)
            
            import json
            existing = []
            if os.path.exists(str(NOTION_LOG)):
                try:
                    with open(str(NOTION_LOG), "r") as f:
                        existing = json.load(f)
                except Exception:
                    existing = []
            
            existing.extend(current_queue)
            with open(str(NOTION_LOG), "w") as f:
...
def main():
    ...
    if args.init:
        pipeline.init_db()
        # ⚡ Bolt: Re-fetching logger as it might have been set up by init_db/log_audit
