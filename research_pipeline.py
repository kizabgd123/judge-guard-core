...
# === CONFIG ===
# ⚡ Bolt: Keep as strings to avoid Path overhead on import
...
NOTION_LOG = Path(NOTION_LOG_STR)
...
class ResearchPipeline:
    ...
    def _ensure_setup(self):
        """⚡ Bolt: Lazy setup of environment, logging, and regex."""
    ...
    def _configure_conn(self, conn):
        """⚡ Bolt: Optimize SQLite for faster writes using WAL mode."""
        conn.execute("PRAGMA journal_mode=WAL")
        conn.execute("PRAGMA synchronous=NORMAL")
        conn.row_factory = None # Placeholder for later sqlite3 setup
        return conn
    ...
    def init_db(self):
        ...
        self.conn = self._configure_conn(self.conn)
    ...
    def cache_verdict(self, action: str, verdict: str):
        ...
        # ⚡ Bolt: Removed explicit commit here, let log_audit handle it for bundling.
    ...
    def sync_to_notion(self):
        ...
        try:
            ...
            # Fallback to file
            self.notion_log.parent.mkdir(exist_ok=True)
...
def main():
    ...
    if args.init:
        pipeline.init_db()
        # pipeline.logger is lazy, it's ok to use it here.
        # But for main, we can just use logging directly if we want to avoid lazy overhead
        # or just use pipeline.logger.
        pipeline.logger.info(f"✅ Database initialized: {pipeline.db_path}")
