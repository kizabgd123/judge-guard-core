...
# === CONFIG ===
# ⚡ Bolt: load_dotenv() at module level ensures DB_PATH from .env is respected.
load_dotenv()
...
# ⚡ Bolt: Lazy-load pattern regex to defer 're' module import.
_PATTERN_RE = None
def get_pattern_re():
    global _PATTERN_RE
...
# ⚡ Bolt: Lazy-load logger to defer 'logging' module initialization.
def get_logger():
    import logging
...
class ResearchPipeline:
    ...
    def _apply_optimizations(self):
        """⚡ Bolt: Enable WAL mode and relax synchronous setting for ~90% faster writes."""
        if self.conn:
    ...
    def cache_verdict(self, action: str, verdict: str):
        ...
        # ⚡ Bolt: Removed redundant self.conn.commit() as log_audit performs the commit,
        # saving one synchronous disk write on every verdict caching.
