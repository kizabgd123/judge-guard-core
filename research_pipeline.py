...
class ResearchPipeline:
    def __init__(self):
        # ⚡ Bolt: Use RLock for thread-safe lazy initialization and nested calls
        ...
        if not self._setup_done:
            with self._lock:
                if not self._setup_done:
                    ...
                    load_dotenv()
                    # Only configure logging if it hasn't been set up yet
    ...
    @property
    def pattern_re(self):
        """⚡ Bolt: Lazy-load and pre-compile regex."""
    ...
    def _apply_optimizations(self, conn):
        """⚡ Bolt: Centralized SQLite performance tuning."""
        import sqlite3
    ...
    def sync_to_notion(self):
        ...
        """
        import json
        # ⚡ Bolt: Ensure environment variables are loaded before os.getenv calls
        self._ensure_setup()
