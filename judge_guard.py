...
# ⚡ Bolt: logging basic config can be light, but we avoid excessive logging on hot paths
...
class JudgeGuard:
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        # ⚡ Bolt: Defer initialization to lazy properties for sub-ms startup
    ...
    def _ensure_env(self):
        """⚡ Bolt: Lazy-load dotenv only once."""
    ...
    @property
    def executor(self):
        """⚡ Bolt: Lazy-load ThreadPoolExecutor to save startup time."""
    ...
    def _discover_brain_path(self) -> Optional[str]:
        ...
        try:
            # ⚡ Bolt: Local import of glob to save startup time
            import glob
