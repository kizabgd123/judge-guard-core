...
class JudgeGuard:
    ...
    # ⚡ Bolt: Class-level constants for faster O(1) attribute access vs O(N) list instantiation
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        # ⚡ Bolt: Use a thread lock for safe lazy initialization of disk-bound or heavy attributes
        ...
        logger.info(f"JudgeGuard v2.0 initialized. (Lazy Discovery Active)")
    ...
    def brain_path(self):
        ...
        if self._brain_path is None:
            with self._init_lock:
                if self._brain_path is None:
                    self._brain_path = self._discover_brain_path() or "unknown"
    ...
    def _load_context(self, max_chars: int = 5000) -> str:
        # ⚡ Bolt: Reduced max_chars to 5000 as downstream logic only uses last 5000.
    ...
    def _check_work_log(self, action_lower: str) -> bool:
        ...
        # ⚡ Bolt: Check last modification time BEFORE reading file
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Lower-case once for reuse in all checks
...
def main():
    # ⚡ Bolt: Load dotenv only if explicitly needed in CLI mode
