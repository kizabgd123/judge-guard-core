...
class JudgeGuard:
    ...
    # ⚡ Bolt: Hoist keywords to class constants to minimize hot-path overhead.
    ...
    @property
    def brain_path(self):
        """⚡ Bolt: Lazy-load brain path using discovery logic."""
        if self._brain_path is None:
            with self._lock:
                if self._brain_path is None:
                    self._brain_path = self._discover_brain_path()
        return self._brain_path

    @property
    def work_log_path(self):
        """⚡ Bolt: Lazy-load work log path using discovery logic."""
        if self._work_log_path is None:
            with self._lock:
                if self._work_log_path is None:
                    self._work_log_path = self._find_work_log()
    ...
    def _is_write_operation(self, action_lower: str) -> bool:
        ...
        Parameters:
            action_lower (str): Lower-cased text of the action description.
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Perform string transformation once for all internal checks
