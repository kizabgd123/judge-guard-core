...
def _ensure_dotenv():
    """⚡ Bolt: Lazy-load dotenv exactly once."""
...
def _ensure_logging():
    """⚡ Bolt: Lazy-configure logging exactly once."""
...
class JudgeGuard:
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        # Caching for WORK_LOG.md
        self._cached_work_log = None
    ...
    def _load_context(self, max_chars: int = 5000) -> str:
        """⚡ Bolt: Optimized mtime-based log retrieval."""
        ...
        try:
            ...
            if mtime == self._cached_work_log_mtime and size == self._cached_work_log_size:
                if self._cached_work_log and len(self._cached_work_log) >= max_chars:
                    return self._cached_work_log[-max_chars:]
    ...
    def _check_work_log(self, action_lower: str) -> bool:
        """Check if WORK_LOG.md was recently updated and contains the action."""
        path = self.work_log_path
        if not path or not os.path.exists(path):
        ...
        # ⚡ Bolt: Restored 'action_lower in last_lines' for correctness
        if (('🟡' in last_lines or 'starting' in last_lines) and (action_lower in last_lines)):
            ...
            self.logger.warning(f"WORK_LOG.md missing '🟡' or action: '{action_lower}'")
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        context = self._load_context(5000)
