...
class JudgeGuard:
    ...
    Verifies every critical step against the 'Standard of Truth'.
    """
    # ⚡ Bolt: Class-level constants for O(1) keyword access
    ...
    def brain_path(self) -> Optional[str]:
        """⚡ Bolt: Lazy-load brain path to reduce initialization overhead."""
        if self._brain_path is None:
            with self._init_lock:
                if self._brain_path is None:
                    self._brain_path = (self._brain_path_override or
                                       os.getenv("BRAIN_PATH") or
                                       self._discover_brain_path() or
                                       "NOT_FOUND")
        return None if self._brain_path == "NOT_FOUND" else self._brain_path

    @property
    def work_log_path(self) -> str:
        """⚡ Bolt: Lazy-load work log path to reduce initialization overhead."""
        if self._work_log_path is None:
            with self._init_lock:
                if self._work_log_path is None:
                    self._work_log_path = (self._work_log_path_override or
                                          os.getenv("WORK_LOG_PATH") or
                                          self._find_work_log())
    ...
    @property
    def gemini(self):
        """⚡ Bolt: Lazy-load GeminiClient to avoid heavy import overhead on startup (thread-safe)."""
    ...
    @property
    def pipeline(self):
        """⚡ Bolt: Lazy-load ResearchPipeline for verdict caching and audit logging (thread-safe)."""
            ...
            with self._init_lock:
                if self._pipeline is None:
                    try:
                        ...
                        self._pipeline = "FAILED"
        return None if self._pipeline == "FAILED" else self._pipeline
    ...
    def _is_dangerous_command(self, action_lower: str) -> bool:
        ...
        ⚡ Bolt: Uses pre-calculated lower-case action for efficiency.
        """
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Pre-calculate lower-case action for efficiency in specialized check methods
