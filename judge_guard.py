...
def _ensure_setup():
    """⚡ Bolt: Defer dotenv and logging setup to reduce import latency."""
    global _setup_done
    if not _setup_done:
        with _global_lock:
            if not _setup_done:
...
class JudgeGuard:
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        # logger.info is deferred until first use to keep init fast.
        # But we call _ensure_setup here to make sure env is ready if we need it.
        # However, to be truly lazy, we can defer it to the properties.

    @property
    def brain_path(self):
        """⚡ Bolt: Lazy-load brain path to avoid expensive glob search on init."""
    ...
    def work_log_path(self):
        """⚡ Bolt: Lazy-load work log path to avoid disk I/O on init."""
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Perform .lower() once and reuse throughout verification
