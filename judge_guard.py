...
# Hoist heavy imports to lazy property scope
load_dotenv = None
...
class JudgeGuard:
    ...
    Verifies every critical step against the 'Standard of Truth'.
    """
    # ⚡ Bolt: Hoist keyword constants to avoid repeated allocation on the hot path
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        logger.info("JudgeGuard v2.0 initialized (lazy properties).")

    def _ensure_dotenv(self):
        """⚡ Bolt: Ensure .env is loaded exactly once, thread-safely."""
        global _DOTENV_LOADED, load_dotenv
    ...
    def brain_path(self) -> Optional[str]:
        ...
        if not self._brain_path and not self._brain_path_searched:
            with self._lock:
                if not self._brain_path and not self._brain_path_searched:
                    self._ensure_dotenv()
    ...
    def _is_write_operation(self, action_lower: str) -> bool:
        ...
        Parameters:
		action_lower (str): Lower-cased action description to inspect.
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Normalize action once for all verification layers
