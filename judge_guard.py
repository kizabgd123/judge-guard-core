...
# Setup logger (Bolt: Consistent module-level logger)
...
def _ensure_setup():
    """⚡ Bolt: Lazy-load dotenv and setup logging exactly once."""
    global _setup_done
    with _global_lock:
        if not _setup_done:
            try:
...
class JudgeGuard:
    ...
    def _load_context(self, max_chars: int = 15000) -> str:
        """⚡ Bolt: Implement mtime-based caching for WORK_LOG.md."""
        ...
        if path and os.path.exists(path):
            try:
                ...
                # ⚡ Bolt: Cache check must respect requested max_chars
                # Invalidate cache if mtime changed OR if requested more than we have (and not all file is cached)
                is_stale = mtime != self._cached_log_mtime
                needs_more = self._cached_log_content is None or (len(self._cached_log_content) < max_chars and len(self._cached_log_content) < file_size)

                if not is_stale and not needs_more:
                    return self._cached_log_content
    ...
    def _is_dangerous_command(self, action_lower: str) -> bool:
        """⚡ Bolt: Use pre-computed lower-case action and hoisted keywords."""
    ...
    def _check_work_log(self, action_lower: str) -> bool:
        """⚡ Bolt: Check if WORK_LOG.md was recently updated, using cached content."""
        path = self.work_log_path
        ...
        # ⚡ Bolt: Use cached content or reload if stale
        last_lines_lower = self._load_context(max_chars=1000).lower()
        
        # Check if this action or 'starting' is in recent log
        if '🟡' in last_lines_lower or 'starting' in last_lines_lower:
            if age_seconds < 120:
                return True
            else:
