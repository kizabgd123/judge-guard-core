...
def _ensure_dotenv():
    """⚡ Bolt: Ensure environment variables are loaded exactly once across instances."""
...
class JudgeGuard:
    ...
    # ⚡ Bolt: Hoisted keywords for hot-path efficiency
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        # Input overrides
        self._brain_path_override = brain_path
        self._work_log_path_override = work_log_path
        ...
        # Work log cache
        self._cached_work_log_content = ""
        self._cached_work_log_mtime = 0.0
        self._cached_work_log_size = 0

        logger.info("JudgeGuard v2.0 initialized (Lazy).")

    @property
    def brain_path(self) -> Optional[str]:
        """⚡ Bolt: Lazy-load brain path with thread safety."""
        if self._brain_path is None:
            with self._lock:
                if self._brain_path is None:
                    _ensure_dotenv()
                    self._brain_path = self._brain_path_override or os.getenv("BRAIN_PATH") or self._discover_brain_path()
        return self._brain_path

    @property
    def work_log_path(self) -> str:
        """⚡ Bolt: Lazy-load work log path with thread safety."""
        if self._work_log_path is None:
            with self._lock:
                if self._work_log_path is None:
                    _ensure_dotenv()
                    self._work_log_path = self._work_log_path_override or os.getenv("WORK_LOG_PATH") or self._find_work_log()
    ...
    def immutable_laws(self) -> str:
        """⚡ Bolt: Lazy-load immutable laws with thread safety."""
    ...
    def _get_log_context(self, max_chars: int = 15000) -> str:
        """⚡ Bolt: Efficient mtime-based log retrieval with caching."""
        ...
        try:
            ...
            # If we already have the tail cached and file hasn't changed, reuse it.
            # We also check if the requested window is larger than our cache.
            if mtime == self._cached_work_log_mtime and size == self._cached_work_log_size:
                if len(self._cached_work_log_content) >= max_chars or len(self._cached_work_log_content) == size:
                    # ⚡ Bolt: Cache update to avoid redundant slicing if same window requested
                    return self._cached_work_log_content[-max_chars:]

            # Cache miss or stale: perform O(1) tail read
            with open(self.work_log_path, "rb") as f:
                ...
                self._cached_work_log_content = content
                self._cached_work_log_mtime = mtime
                self._cached_work_log_size = file_size
                return content
        except Exception as e:
            logger.debug(f"Log retrieval error: {e}")
            return "(Error loading context)"

    def _load_context(self, max_chars: int = 15000) -> str:
        """Backwards compatible wrapper for _get_log_context."""
    ...
    def _check_work_log(self, action: str) -> bool:
        ...
        # ⚡ Bolt: Use cached content to avoid redundant disk read if called twice (e.g. verify + essence)
        # However, for check_work_log we always want fresh mtime to check 'age_seconds'.
        ...
        try:
            # ⚡ Bolt: Reuse the cached tail retrieval logic
            last_lines = self._get_log_context(max_chars=1000).lower()
