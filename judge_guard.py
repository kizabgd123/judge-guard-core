...
class JudgeGuard:
    ...
    # ⚡ Bolt: Class-level constants for efficient keyword checking
    ...
    def brain_path(self) -> Optional[str]:
        """⚡ Bolt: Lazy-load brain path to avoid globbing on every startup."""
    ...
    def work_log_path(self) -> str:
        """⚡ Bolt: Lazy-load work log path to avoid directory scanning on every startup."""
    ...
    def immutable_laws(self) -> str:
        """⚡ Bolt: Lazy-load rules to avoid disk read on every startup."""
    ...
    def _check_work_log(self, action_lower: str) -> bool:
        ...
        try:
            # ⚡ Bolt: Efficient O(1) tail retrieval
            with open(work_log_path, 'rb') as f:
                f.seek(0, 2)
                file_size = f.tell()
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Lower the action once for all keyword checks
