...
class JudgeGuard:
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        # ⚡ Bolt: Use lazy-loaded properties for heavy dependencies
        self._gemini = None
        self._pipeline = None
        self._bridge = None
        self._block_judge_class = None

        logger.info(f"JudgeGuard v2.0 initialized. Brain: {self.brain_path}")

    @property
    def gemini(self):
        """⚡ Bolt: Lazy-load GeminiClient."""
        if self._gemini is None:
            try:
                ...
                self._gemini = False # Sentinel for not available
        return self._gemini if self._gemini is not False else None

    @property
    def pipeline(self):
        """⚡ Bolt: Lazy-load ResearchPipeline."""
    ...
    @property
    def block_judge_class(self):
        """⚡ Bolt: Lazy-load BlockJudge class."""
        if self._block_judge_class is None:
            try:
                from src.antigravity_core.judge_flow import BlockJudge
                self._block_judge_class = BlockJudge
            except ImportError as e:
                logger.warning(f"⚠️ BlockJudge not available: {e}")
                self._block_judge_class = False
        return self._block_judge_class if self._block_judge_class is not False else None
    ...
    def close(self):
        ...
        # Check _pipeline specifically to avoid triggering lazy load during shutdown
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        if not self.gemini or not self.block_judge_class:
        ...
        if self.bridge:
            ...
            self.bridge.push_verdict("Judging...", "PENDING", status_msg)
        ...
        judge = self.block_judge_class(criteria, client=self.gemini)
        passed = judge.evaluate(f"ACTION: {current_action}")
        
        if passed:
            print(f"✅ JudgeGuard: Action '{current_action}' APPROVED.")
            if self.bridge:
                self.bridge.push_verdict(current_action, "PASSED", "Approved (Unified Verification)")
