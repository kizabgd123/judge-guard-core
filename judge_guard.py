...
# ⚡ Bolt: Lazy imports for heavy dependencies
# from concurrent.futures import ThreadPoolExecutor  -> Lazy loaded
# import glob -> Lazy loaded
# from dotenv import load_dotenv -> Lazy loaded
...
class JudgeGuard:
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        # ⚡ Bolt: Context caching
        self._cached_context = None
        self._cached_mtime = -1
        self._cached_max_chars = -1

        # ⚡ Bolt: Initialization is now lazy. Paths are discovered on first access.
        # logger.info(f"JudgeGuard v2.0 initialized.")

    def _ensure_dotenv(self):
        """⚡ Bolt: Load .env exactly once before accessing env vars."""
        ...
        if self._brain_path is None:
            with self._lock:
                if self._brain_path is None:
                    ...
                    if path:
                        logger.info(f"JudgeGuard: Discovered Brain at {path}")
    ...
    def _load_context(self, max_chars: int = 15000) -> str:
        """⚡ Bolt: Context loading with mtime-based caching."""
        ...
        try:
            ...
            # Use cache if mtime and requested size match
            if mtime == self._cached_mtime and max_chars <= self._cached_max_chars:
            ...
            with open(self.work_log_path, "rb") as f:
                ...
                self._cached_max_chars = max_chars
                return context
        except Exception:
    ...
    def _check_work_log(self, action: str) -> bool:
        ...
        # ⚡ Bolt: Reuse cached context from _load_context or trigger it
