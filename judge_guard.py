...
# Global lock for thread-safe dotenv loading
_dotenv_lock = threading.Lock()
_dotenv_loaded = False

def _ensure_dotenv():
    """⚡ Bolt: Lazy-load and ensure dotenv is loaded exactly once."""
    global _dotenv_loaded, _dotenv
...
class JudgeGuard:
    ...
    # ⚡ Bolt: Hoisted keyword constants to minimize overhead in hot path
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        self._pipeline = None
        self._executor = None
        self._lock = threading.RLock() # ⚡ Bolt: Re-entrant lock for thread-safe lazy property initialization
        ...
        if self._executor is None:
            with self._lock:
                if self._executor is None:
                    ...
                    self._executor = _concurrent_futures.ThreadPoolExecutor(max_workers=1)
        return self._executor

    @property
    def brain_path(self):
        """⚡ Bolt: Lazy-resolve brain path."""
    ...
    def work_log_path(self):
        """⚡ Bolt: Lazy-resolve work log path."""
    ...
    def _discover_brain_path(self) -> Optional[str]:
        ...
        try:
            ...
            brain_dirs = _glob.glob(os.path.join(base_path, "*-*-*-*-*"))
            if not brain_dirs:
                return None
    ...
    def _load_context(self, max_chars: int = 15000) -> str:
        """Load recent context from work log."""
    ...
    def _is_dangerous_command(self, action_lower: str) -> bool:
        ...
        Parameters:
            action_lower (str): Text of the action (lower-cased) to inspect.
    ...
    def _is_write_operation(self, action_lower: str) -> bool:
        ...
        Parameters:
		action_lower (str): Freeform action description (lower-cased) to inspect for write/edit-related keywords.
        
        Returns:
		True if keywords indicate modification, False otherwise.
        """
    ...
    def _check_work_log(self, current_action: str, action_lower: str) -> bool:
            ...
            with open(self.work_log_path, 'rb') as f:
                ...
                if '🟡' in last_lines or 'starting' in last_lines:
                    if action_lower in last_lines or "starting" in last_lines:
                        ...
                        logger.warning(f"WORK_LOG.md does not contain the current action: {current_action}")
                else:
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Lowercase once for all heuristic checks
        ...
        if not self._check_work_log(current_action, action_lower):
            return False
