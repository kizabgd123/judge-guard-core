...
# ⚡ Bolt: Global sentinel and lock for thread-safe dotenv loading
...
class JudgeGuard:
    ...
    # ⚡ Bolt: Hoist keyword lists to class constants to avoid repeated allocation
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        # ⚡ Bolt: Sentinels to track if path discovery has been attempted
        self._brain_path_searched = False
        self._work_log_path_searched = False

        logger.info("JudgeGuard v2.0 initialized (deferred loading enabled).")

    def _ensure_dotenv(self):
        """⚡ Bolt: Ensure environment variables are loaded exactly once."""
    ...
    def work_log_path(self) -> str:
        ...
        if not self._work_log_path and not self._work_log_path_searched:
            with self._lock:
                if not self._work_log_path and not self._work_log_path_searched:
                    self._ensure_dotenv()
                    self._work_log_path = os.getenv("WORK_LOG_PATH") or self._find_work_log()
                    self._work_log_path_searched = True
        return self._work_log_path or os.path.join(os.getcwd(), "WORK_LOG.md")
