...
def _ensure_dotenv():
    """⚡ Bolt: Lazy-load dotenv only when environment variables are needed."""
    global _ENV_LOADED
    if not _ENV_LOADED:
        with _ENV_LOCK:
            if not _ENV_LOADED:
...
class JudgeGuard:
    ...
    # ⚡ Bolt: Hoist keyword lists to class constants for O(1) access and better readability
    ...
    @property
    def executor(self):
        """⚡ Bolt: Lazy-load ThreadPoolExecutor only when background tasks are needed."""
    ...
    @property
    def rules_path(self):
        """⚡ Bolt: Lazy property for rules path resolution."""
    ...
    def immutable_laws(self):
        """⚡ Bolt: Lazy property for loading rules from disk."""
    ...
    def _get_log_context(self, max_chars: int = 15000) -> str:
        """
        ⚡ Bolt: Optimized tail retrieval with mtime caching.
        Avoids redundant disk I/O if the file hasn't changed.
        """
        ...
        try:
            ...
            # If mtime is the same and we have enough chars cached, return from cache
            if mtime == self._cached_work_log_mtime and self._cached_work_log_tail and self._cached_work_log_size >= max_chars:
    ...
    def _is_dangerous_command(self, action_lower: str) -> bool:
        ...
        ⚡ Bolt: Expects pre-lowercased action string.
        """
    ...
    def _check_work_log(self, action: str) -> bool:
        ...
        # ⚡ Bolt: Use optimized log retrieval (mtime cached)
        # We need at least 1000 chars for the check
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Normalize action string once for all keyword and logic checks
