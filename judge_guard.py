...
# ⚡ Bolt: Global lock and sentinel for thread-safe lazy environment loading
...
class JudgeGuard:
    ...
    # ⚡ Bolt: Hoist keywords to constants to avoid repeated list instantiation
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        # ⚡ Bolt: Defer everything to lazy properties to achieve sub-1ms initialization
        ...
        # ⚡ Bolt: Caching for WORK_LOG context to avoid redundant disk I/O
        self._log_cache = {
            "mtime": 0,
            "content": "",
            "path": ""
    ...
    def rules_path(self) -> str:
        """⚡ Bolt: Lazy resolution of rules path."""
    ...
    def _get_log_context(self, max_chars: int = 15000) -> str:
        """⚡ Bolt: Buffered tail retrieval with mtime caching."""
        ...
        try:
            ...
            # Use cache if mtime matches AND we have enough chars
            if (self._log_cache["path"] == path and
                self._log_cache["mtime"] == mtime and
                len(self._log_cache["content"]) >= max_chars):
    ...
    def _is_dangerous_command(self, action_lower: str) -> bool:
        """⚡ Bolt: Use pre-compiled keywords and normalized input."""
    ...
    def _check_work_log(self, action_lower: str) -> bool:
        ...
        # ⚡ Bolt: Use cached log context to avoid redundant read
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Normalize action once to save repeated lower() calls
