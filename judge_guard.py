...
class JudgeGuard:
    ...
    Verifies every critical step against the 'Standard of Truth'.
    """
    # ⚡ Bolt: Hoist keyword constants to class level to avoid redundant instantiation.
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        # ⚡ Bolt: Thread-safe lazy properties initialization
    ...
    @property
    def rules_path(self):
        """⚡ Bolt: Lazy property for rules path discovery."""
    ...
    def immutable_laws(self):
        """⚡ Bolt: Lazy property for rules loading."""
    ...
    @property
    def gemini(self):
        """⚡ Bolt: Lazy-load GeminiClient with thread safety."""
    ...
    @property
    def pipeline(self):
        """⚡ Bolt: Lazy-load ResearchPipeline with thread safety."""
    ...
    def _load_context(self, max_chars: int = 15000) -> str:
        ...
        try:
            # ⚡ Bolt: Use mtime-based caching to avoid redundant disk I/O
            current_mtime = os.path.getmtime(self.work_log_path)
            if current_mtime <= self._work_log_mtime and self._work_log_cache:
                return self._work_log_cache
            ...
            with open(self.work_log_path, "rb") as f:
                ...
                f.seek(-to_read, 2)
                self._work_log_cache = f.read().decode('utf-8', errors='ignore')
                self._work_log_mtime = current_mtime
                return self._work_log_cache
        except Exception:
            pass
        return "(No work log context)"
    ...
    def _is_dangerous_command(self, action: str) -> bool:
        ...
        Parameters:
            action (str): Text of the action to inspect.
        
        Returns:
            bool: `True` if any dangerous pattern is present in `action`, `False` otherwise.
        """
        # ⚡ Bolt: Use class-level constant
        return any(k in action for k in self.DANGEROUS_KEYWORDS)
    ...
    def _check_work_log(self, action: str) -> bool:
        ...
        current_mtime = os.path.getmtime(self.work_log_path)
        now = time.time()
        age_seconds = now - current_mtime
        
        # Read last few lines to check if action was logged
        try:
            # ⚡ Bolt: Use mtime-based caching to avoid redundant disk I/O
            if current_mtime <= self._work_log_mtime and self._work_log_cache:
                last_lines = self._work_log_cache[-1000:].lower()
            else:
                # ⚡ Bolt: Efficient O(1) tail retrieval
                with open(self.work_log_path, 'rb') as f:
                    f.seek(0, 2)
                    file_size = f.tell()
                    # We read 15000 in _load_context, but here only 1000.
                    # To share cache, we should read the larger amount.
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Lower-case once and reuse to avoid redundant .lower() calls in helpers.
