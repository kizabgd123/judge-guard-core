...
# Module-level thread-safe setup
_global_lock = threading.RLock()
_setup_done = False

def _ensure_setup():
    """⚡ Bolt: Defer environment and logging setup to reduce import overhead."""
    global _setup_done
    if not _setup_done:
        with _global_lock:
            if not _setup_done:
                ...
                logger.info("JudgeGuard v2.0 lazy setup complete.")
                _setup_done = True
...
class JudgeGuard:
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        # ⚡ Bolt: Context caching for WORK_LOG.md
    ...
    @property
    def brain_path(self):
        """⚡ Bolt: Lazy-load brain path to avoid expensive glob on startup."""
    ...
    def work_log_path(self):
        """⚡ Bolt: Lazy-load work log path to avoid disk I/O on startup."""
    ...
    def _load_context(self, max_chars: int = 15000) -> str:
        """⚡ Bolt: Load context from WORK_LOG.md with mtime-based caching."""
        ...
        try:
            ...
            if (abs(float(stat.st_mtime) - float(self._cached_log_mtime)) < 0.01 and
                (self._cached_log_size >= max_chars or self._cached_log_size == stat.st_size)):
            ...
            logger.warning(f"⚠️ Error reading WORK_LOG.md: {e}")
    ...
    def _is_dangerous_command(self, action_lower: str) -> bool:
        ...
        Parameters:
            action_lower (str): Pre-lowered text of the action to inspect.
    ...
    def _is_write_operation(self, action_lower: str) -> bool:
        ...
        Parameters:
            action_lower (str): Pre-lowered action description.
        
        Returns:
            True if it contains write-related keywords, False otherwise.
        """
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Pre-calculate lowercase action once for all subsequent checks
