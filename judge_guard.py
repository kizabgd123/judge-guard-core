...
# ⚡ Bolt: Global re-entrant lock for thread-safe lazy property initialization
...
class JudgeGuard:
    ...
    # ⚡ Bolt: Class-level constants for efficient keyword matching
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        # Initialized with provided paths or None for lazy discovery
        ...
        # Only log minimal info to keep startup clean
        logger.debug("JudgeGuard v2.0 initialized (Lazy Mode).")

    def _ensure_dotenv(self):
        """⚡ Bolt: Lazy-load dotenv to avoid I/O on import."""
    ...
    @property
    def executor(self):
        """⚡ Bolt: Lazy-load executor to avoid resource allocation if not needed."""
    ...
    def brain_path(self) -> Optional[str]:
        """⚡ Bolt: Lazy-discover brain path on first access."""
    ...
    def work_log_path(self) -> str:
        """⚡ Bolt: Lazy-find work log path on first access."""
    ...
    def immutable_laws(self) -> str:
        """⚡ Bolt: Lazy-load rules on first access."""
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Cache lower-case action once to avoid redundant .lower() calls
        ...
        # Cache pipeline reference locally to avoid repeated property lookups
        ...
        is_shell_command = "run_command" in current_action_lower or "shell" in current_action_lower
