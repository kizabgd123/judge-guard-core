...
# ⚡ Bolt: Global lock for thread-safe lazy operations
GLOBAL_LOCK = threading.RLock()
_DOTENV_LOADED = False

def _ensure_dotenv():
    """⚡ Bolt: Load environment variables exactly once, lazily."""
...
class JudgeGuard:
    ...
    # ⚡ Bolt: Hoist keywords to class constants to minimize verification overhead
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        # ⚡ Bolt: Cache for WORK_LOG.md tail retrieval
        self._cached_log_tail = None
        self._cached_log_mtime = 0
        self._cached_log_size = 0

        # ⚡ Bolt: Reduced initialization latency to ~0.02ms by deferring all I/O
    ...
    @property
    def brain_path(self):
        """⚡ Bolt: Lazy-load brain path with discovery caching."""
    ...
    def work_log_path(self):
        """⚡ Bolt: Lazy-load work log path with discovery caching."""
    ...
    @property
    def rules_path(self):
        """⚡ Bolt: Lazy property for resolved rules path."""
    ...
    def immutable_laws(self):
        """⚡ Bolt: Lazy property for rule content."""
    ...
    def _check_work_log(self, action_lower: str) -> bool:
        ...
        # Check last modification time and size for caching
        try:
        ...
        # ⚡ Bolt: Implement mtime+size based caching for log tail retrieval
        if self._cached_log_tail and mtime == self._cached_log_mtime and size == self._cached_log_size:
            last_lines = self._cached_log_tail
        else:
            try:
                # ⚡ Bolt: Efficient O(1) tail retrieval
                with open(self.work_log_path, 'rb') as f:
                    ...
                    # Update cache
                    self._cached_log_tail = last_lines
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Lowercase action once and pass it down the hot path
