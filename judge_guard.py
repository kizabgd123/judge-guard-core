...
class JudgeGuard:
    ...
    # ⚡ Bolt: Use class-level keywords to avoid instance creation overhead
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        # ⚡ Bolt: Minimal init. Defer path discovery and rule loading to lazy properties.
        ...
        # Cache for WORK_LOG.md tail to avoid redundant disk I/O
        self._work_log_cache = None
        self._work_log_mtime = -1
        self._work_log_cached_size = 0

        logger.info("JudgeGuard v2.0 initialized (Lazy).")

    def _ensure_dotenv(self):
        """⚡ Bolt: Guarantee environment variables are loaded exactly once and only when needed."""
    ...
    @property
    def executor(self):
        """⚡ Bolt: Lazy-load ThreadPoolExecutor only if a background task is triggered."""
    ...
    def _load_context(self, max_chars: int = 15000, mtime: Optional[float] = None) -> str:
        path = self.work_log_path
        if path and os.path.exists(path):
            try:
                # ⚡ Bolt: Implement mtime-based caching for WORK_LOG.md tail retrieval
                if mtime is None:
                    mtime = os.path.getmtime(path)

                if mtime == self._work_log_mtime and self._work_log_cached_size >= max_chars:
                    # Return slice of cached buffer if large enough
                ...
                with open(path, "rb") as f:
                    ...
                    self._work_log_cached_size = max_chars
                    return content
            except Exception:
                pass
    ...
    def _is_dangerous_command(self, action_lower: str) -> bool:
        """⚡ Bolt: Use pre-hoisted lowercase action and class-level keywords."""
    ...
    def _check_work_log(self, action_lower: str) -> bool:
        ...
        try:
            ...
            # ⚡ Bolt: Reuse the cached tail content if mtime matches
            last_lines = self._load_context(max_chars=1000, mtime=mtime).lower()
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Hoist string normalization to avoid repeated .lower() calls
