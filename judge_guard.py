...
class JudgeGuard:
    ...
    # ⚡ Bolt: Hoist keyword constants for hot-path performance
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        # Initialization is now lazy. Logger will report on first access.
    ...
    @property
    def executor(self):
        """⚡ Bolt: Lazy-load ThreadPoolExecutor to save resources on short-lived runs."""
        ...
        if self._brain_path is None and not self._brain_path_searched:
            with self._lock:
                if self._brain_path is None and not self._brain_path_searched:
                    ...
                    if self._brain_path:
                        logger.info(f"JudgeGuard: Discovered brain at {self._brain_path}")
        return self._brain_path

    @property
    def work_log_path(self):
        """⚡ Bolt: Lazy-load and find WORK_LOG.md."""
    ...
    def _is_dangerous_command(self, action_lower: str) -> bool:
        """⚡ Bolt: Use pre-lowercased action for optimized keyword check."""
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Lowercase once at the start of the hot path
