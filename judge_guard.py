...
# ⚡ Bolt: Global lock and sentinel for thread-safe lazy setup
_global_lock = threading.RLock()
_setup_done = False

def _ensure_setup():
    """⚡ Bolt: Lazy-load environment variables and configure logging exactly once."""
    global _setup_done
    if not _setup_done:
        with _global_lock:
            if not _setup_done:
...
class JudgeGuard:
    ...
    Verifies every critical step against the 'Standard of Truth'.
    """
    # ⚡ Bolt: Hoist keywords to class constants for O(1) access
    ...
    def brain_path(self) -> Optional[str]:
        """⚡ Bolt: Lazy-load brain path to defer glob operations."""
        if not self._brain_path_searched:
            with self._lock:
                if not self._brain_path_searched:
                    ...
                    logger.info(f"JudgeGuard: Discovered brain path: {self._brain_path}")
        return self._brain_path

    @property
    def work_log_path(self) -> str:
        """⚡ Bolt: Lazy-load work log path to defer disk search."""
    ...
    def _is_dangerous_command(self, action_lower: str) -> bool:
        ...
        ⚡ Bolt: Accepts pre-lowered action string for efficiency.
        """
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Pre-calculate lowercase action once for all internal checks
