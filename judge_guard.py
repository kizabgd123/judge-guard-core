...
class JudgeGuard:
    ...
    @property
    def brain_path(self):
        """⚡ Bolt: Lazy-load brain path (thread-safe)."""
    ...
    def work_log_path(self):
        """⚡ Bolt: Lazy-load work log path (thread-safe)."""
        if self._work_log_path is None:
            with self._init_lock:
                if self._work_log_path is None:
                    self._work_log_path = os.getenv("WORK_LOG_PATH") or self._find_work_log()
                    logger.info(f"JudgeGuard: Work log path discovered: {self._work_log_path}")
        return self._work_log_path

    @property
    def immutable_laws(self):
        """⚡ Bolt: Lazy-load rules from MASTER_ORCHESTRATION.md (thread-safe)."""
    ...
    def _is_dangerous_command(self, action_lower: str) -> bool:
        ...
        ⚡ Bolt: Uses pre-lowered action string and class constants.
        """
    ...
    def _check_work_log(self, action: str) -> bool:
        ...
        # ⚡ Bolt: Local cache of path to avoid repeated property/lock overhead
        path = self.work_log_path
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Lower-case action once for all subsequent keyword checks
