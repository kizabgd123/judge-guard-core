...
# ⚡ Bolt: Global lock and sentinel for thread-safe lazy dotenv loading
...
class JudgeGuard:
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        # ⚡ Bolt: Store args for lazy resolution
        ...
        self._executor_internal = None
        self._gemini = None
        self._pipeline = None

        # ⚡ Bolt: Thread-safety for lazy property initialization
        self._lock = threading.RLock()

        # ⚡ Bolt: mtime-based cache for work log tail
        self._last_work_log_mtime = 0
        self._cached_work_log_tail = None
        self._cached_work_log_size = 0

        # ⚡ Bolt: Defer path discovery and rule loading to properties
        logger.info("JudgeGuard v2.0 initialized (Lazy).")

    @property
    def executor(self):
        """⚡ Bolt: Lazy-load ThreadPoolExecutor to reduce startup latency."""
        if self._executor_internal is None:
            with self._lock:
                if self._executor_internal is None:
                    from concurrent.futures import ThreadPoolExecutor
                    self._executor_internal = ThreadPoolExecutor(max_workers=1)
        return self._executor_internal
    ...
    def close(self):
        ...
        if hasattr(self, "_executor_internal") and self._executor_internal:
            self._executor_internal.shutdown(wait=False)
        if hasattr(self, "_pipeline") and self._pipeline:
            self._pipeline.close()

    @property
    def brain_path(self) -> Optional[str]:
        """⚡ Bolt: Lazy-load brain path from environment or discovery."""
    ...
    def work_log_path(self) -> str:
        """⚡ Bolt: Lazy-load work log path from environment or discovery."""
    ...
    def _load_context(self, max_chars: int = 15000) -> str:
        """
        ⚡ Bolt: Load recent context from WORK_LOG.md with mtime-based caching.
        """
        ...
        try:
            ...
            # ⚡ Bolt: Check mtime AND requested size to avoid truncation bug
            is_mtime_same = mtime == self._last_work_log_mtime
            has_enough_data = self._cached_work_log_size >= max_chars

            if self._cached_work_log_tail and is_mtime_same and has_enough_data:
                return self._cached_work_log_tail[-max_chars:]
            ...
            with open(path, "rb") as f:
                ...
                # Update cache
                self._cached_work_log_tail = content
                self._cached_work_log_size = to_read
                self._last_work_log_mtime = mtime
                return content
        except Exception:
            pass
        return "(No work log context)"
    ...
    def _check_work_log(self, action: str) -> bool:
        ...
        try:
            # ⚡ Bolt: Use mtime-cached tail if available (even if smaller, as we only need 1000)
            if self._cached_work_log_tail and mtime == self._last_work_log_mtime:
                last_lines = self._cached_work_log_tail[-1000:].lower()
            else:
                # ⚡ Bolt: Efficient O(1) tail retrieval
                with open(path, 'rb') as f:
                    f.seek(0, 2)
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Normalize action string once for all checks
