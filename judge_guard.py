...
def _ensure_setup():
    """⚡ Bolt: Lazy-load environment and logging to reduce import latency."""
    global _setup_done
    if not _setup_done:
        with _global_lock:
            if not _setup_done:
...
class JudgeGuard:
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        # ⚡ Bolt: Defer path discovery and logging to avoid I/O in __init__
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Pre-calculate lower-case action for reuse across all checks
