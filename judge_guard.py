...
def _ensure_setup():
    """⚡ Bolt: Defer dotenv and logging setup to first use to reduce import time."""
    global _setup_done
    if not _setup_done:
        with _global_lock:
            if not _setup_done:
...
class JudgeGuard:
    ...
    # ⚡ Bolt: Hoisted keywords for faster hot-path checks
            ...
            with self._lock:
                if self._gemini is None:
                    try:
                        ...
                        _ensure_setup()
                        logging.getLogger(__name__).warning(f"⚠️ GeminiClient not available: {e}")
        return self._gemini
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                _ensure_setup()
                                logging.getLogger(__name__).warning(f"⚠️ Failed to initialize ResearchPipeline: {e}")
                                self._pipeline = None
                    except ImportError as e:
                        _ensure_setup()
                        logging.getLogger(__name__).warning(f"⚠️ ResearchPipeline not available: {e}")
    ...
    def _load_context(self, max_chars: int = 5000) -> str:
        """⚡ Bolt: Reduced default window to 5000 chars to minimize disk I/O."""
    ...
    def _is_dangerous_command(self, action_lower: str) -> bool:
        """⚡ Bolt: Uses hoisted keywords and pre-lowercased action string."""
    ...
    def _sync_to_notion(self):
        ...
        try:
            ...
            _ensure_setup()
            logging.getLogger(__name__).error(f"⚠️ Notion background sync failed: {e}")
    ...
    def _check_work_log(self, action_lower: str) -> bool:
        ...
        if not self.work_log_path or not os.path.exists(self.work_log_path):
            _ensure_setup()
            logging.getLogger(__name__).error("🛑 WORK_LOG.md not found. Required for action verification.")
        ...
        try:
            ...
            with open(self.work_log_path, 'rb') as f:
                ...
                # ⚡ Bolt: Restored action_lower check for correctness
                if ('🟡' in last_lines or 'starting' in last_lines) and action_lower in last_lines:
                    if age_seconds < 120:
                        return True
                    else:
                        _ensure_setup()
                        logging.getLogger(__name__).warning(f"WORK_LOG.md is stale ({age_seconds:.1f}s old). Action must be logged recently.")
                else:
                    _ensure_setup()
                    logging.getLogger(__name__).warning("WORK_LOG.md does not contain '🟡' or 'Starting' indicators + action in the last 1000 chars.")

        except Exception as e:
            _ensure_setup()
            logging.getLogger(__name__).error(f"⚠️ Error reading WORK_LOG.md: {e}")
            return False
        
        print("🛑 WORK_LOG.md not updated recently. Required format:")
        print(f'   echo "🟡 Starting {action_lower}" >> WORK_LOG.md')
        return False
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        if self._is_dangerous_command(action_lower):
            ...
            _ensure_setup()
            logging.getLogger(__name__).error(f"Layer 00 Block: {msg}")
            if bridge_available:
        ...
        if is_research_phase and is_shell_command:
            ...
            _ensure_setup()
            logging.getLogger(__name__).warning(f"Layer 1 Block: {msg}")
            if bridge_available:
                bridge.push_verdict(current_action, "BLOCKED", msg)
            print(f"🛑 JudgeGuard: {msg}")
            return False
        ...
        _ensure_setup()
        logging.getLogger(__name__).info(f"Consolidated Verification (Write: {is_write})...")
