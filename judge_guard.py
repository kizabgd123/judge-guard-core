...
# ⚡ Bolt: Global lock and sentinel to ensure dotenv is loaded exactly once across instances.
...
class JudgeGuard:
    ...
    # ⚡ Bolt: Hoist keywords to class constants to minimize overhead in hot path
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        # ⚡ Bolt: Use a lock for thread-safe lazy property initialization
        ...
        # ⚡ Bolt: Caching for WORK_LOG.md content
    ...
    def _ensure_dotenv(self):
        """⚡ Bolt: Load environment variables exactly once."""
    ...
    def brain_path(self) -> Optional[str]:
        """⚡ Bolt: Lazy-load brain path using disk-intensive glob discovery."""
    ...
    def work_log_path(self) -> str:
        """⚡ Bolt: Lazy-load work log path with parent directory traversal."""
    ...
    def immutable_laws(self) -> str:
        """⚡ Bolt: Lazy-load rules from disk on demand."""
    ...
    @property
    def executor(self):
        """⚡ Bolt: Lazy-load ThreadPoolExecutor to avoid early import overhead."""
    ...
    def _get_log_context(self, max_chars: int = 5000) -> str:
        """⚡ Bolt: Thread-safe mtime-based caching for log retrieval."""
        ...
        try:
            ...
            with self._lock:
                # If file hasn't changed, reuse cache
                if self._cached_log_content is not None and mtime <= self._cached_log_mtime:
                    # If we have enough chars in cache, return them
                    if len(self._cached_log_content) >= max_chars or len(self._cached_log_content) == self._cached_log_size:
    ...
    def _load_context(self, max_chars: int = 5000) -> str:
        """⚡ Bolt: Use cached log context (default window reduced to 5000 for speed)."""
    ...
    def _check_work_log(self, action_lower: str) -> bool:
        ...
        # ⚡ Bolt: Use os.stat to get mtime and size once
        try:
            stat = os.stat(path)
            mtime = stat.st_mtime
            now = time.time()
            ...
            # ⚡ Bolt: Re-use log context from cache if possible
            ...
            if last_lines:
                ...
                if '🟡' in last_lines or 'starting' in last_lines:
                    if age_seconds < 120:
                        # ⚡ Bolt: Check that the current action (or part of it) is in the log
                        if action_lower in last_lines or 'starting' in last_lines:
                            return True
                    else:
    ...
    def verify_action(self, current_action: str) -> bool:
        ...
        """
        # ⚡ Bolt: Single lower() transformation for all internal checks
