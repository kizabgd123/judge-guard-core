...
# ⚡ Bolt: Constant Hoisting for hot-path speed
...
class JudgeGuard:
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        ...
        self._gemini = None
        self._pipeline = None
        # Sentinel to avoid redundant connection attempts if initialization fails
        self._pipeline_initialized = False

        # ⚡ Bolt: Work log tail caching
        self._log_cache = {
            "mtime": 0,
            "content": "",
            "size_read": 0
    ...
    def _ensure_dotenv(self):
        """⚡ Bolt: Load .env exactly once and only when needed."""
            ...
            with self._lock:
                if self._pipeline is None and not self._pipeline_initialized:
                    try:
                        ...
                        self._pipeline_initialized = True # Don't retry every time
    ...
    def _get_log_context(self, max_chars: int = 15000) -> str:
        """⚡ Bolt: Efficient O(1) tail retrieval with mtime-based caching."""
        ...
        try:
            ...
            # Check if cache is valid and contains enough data
            if (self._log_cache["mtime"] == mtime and
                self._log_cache["size_read"] >= max_chars):
                # Return slice of cached content
    ...
    def _check_work_log(self, action: str) -> bool:
        ...
        # ⚡ Bolt: Use cached log context to avoid redundant disk read
        last_lines = self._get_log_context(1000).lower()

        if not last_lines:
             logger.warning("WORK_LOG.md is empty or could not be read.")
             return False
