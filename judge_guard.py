...
def _ensure_setup():
    """⚡ Bolt: Lazy-load environment and logging once."""
    global _setup_done
    if not _setup_done:
        with _global_lock:
            if not _setup_done:
...
class JudgeGuard:
    ...
    def __init__(self, brain_path: Optional[str] = None, work_log_path: Optional[str] = None):
        # ⚡ Bolt: Offload all discovery and setup to lazy properties
        ...
        # ⚡ Bolt: Deferred initialization log
        if brain_path:
            logger.info(f"JudgeGuard v2.0 initialized. Brain: {brain_path}")
    ...
    @property
    def brain_path(self):
        """⚡ Bolt: Lazy-load brain path with auto-discovery."""
    ...
    def work_log_path(self):
        """⚡ Bolt: Lazy-load work log path with auto-discovery."""
    ...
    def _is_dangerous_command(self, action_lower: str) -> bool:
        ...
        Parameters:
            action_lower (str): Lower-cased text of the action to inspect.
        
        Returns:
            bool: `True` if any dangerous pattern is present in `action_lower`, `False` otherwise.
        """
    ...
    def _is_write_operation(self, action_lower: str) -> bool:
        ...
        Parameters:
		action_lower (str): Lower-cased freeform action description.
    ...
    def _check_work_log(self, action_lower: str) -> bool:
        ...
        work_log = self.work_log_path
        if not work_log or not os.path.exists(work_log):
